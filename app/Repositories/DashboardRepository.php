...
class DashboardRepository implements DashboardInterface {
    ...
    public function borderDashboardData(){
        ...
        $borderConsts = Bill::with('billStatus')
                    ->selectRaw("type, note, amount, bill_month, bill_status_id, IF(border_id IS NULL, 'without_user', 'with_user') AS user_type")
                    ->whereMonth('bill_month', Carbon::now())
                    ->where('hostel_id', $authUser->hostel_id)
                    ->orderBy('id', 'ASC')
                    ->get();

        $data['borderCostData'] = $borderConsts->where('user_type', 'with_user');
